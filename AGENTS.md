...
- **Prompt-инъекции:** текст из веба/транскрипта недоверенный. `userPrompt` оборачивает его
  (и заголовок) в маркеры `⟦SOURCE <код>⟧…⟦/SOURCE <код>⟧` со случайным кодом на запрос, а
  `SAFETY_RULES` велят игнорировать инструкции изнутри блока. Системный промпт собирается
  как `composeSystemPrompt(task) = task + SAFETY_RULES`: **task-часть** — это то, что
  заменяет `SYSTEM_PROMPT_FILE`, а **защита живёт в коде и подмешивается всегда**, поэтому
  кастомный промпт не может её потерять. Правила и обёртка ссылаются на общую константу
  `SOURCE_TAG` — не разъезжайся. Без действий/секретов в контексте инъекция максимум портит
  вывод: это снижение риска, не абсолют.
