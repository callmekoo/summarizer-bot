...
- **Prompt-инъекции:** текст из веба/транскрипта недоверенный. `userPrompt` оборачивает его
  ...
  системный промпт велит игнорировать инструкции изнутри блока. Без действий/секретов в
  контексте инъекция максимум портит вывод — это снижение риска, не абсолют; правило держи
  и в системном промпте, и в обёртке (защита в глубину).
